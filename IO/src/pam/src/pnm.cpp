...
std::ofstream *PpmWriter::openPpm(const char *filename, int64_t w, int64_t h, VideoStitch::ThreadSafeOstream *err) {
...
std::ofstream *PpmWriter::openPam(const char *filename, int64_t w, int64_t h, VideoStitch::ThreadSafeOstream *err) {
  std::ofstream *ofs = openGeneric(filename, err);
  if (ofs) {
    *ofs << "P7\nWIDTH " << w << "\nHEIGHT " << h << "\nDEPTH 4\nMAXVAL 255\nTUPLTYPE RGB_ALPHA\nENDHDR\n";
  }
...
std::ofstream *PpmWriter::openPgm(const char *filename, int64_t w, int64_t h, VideoStitch::ThreadSafeOstream *err) {
  std::ofstream *ofs = openGeneric(filename, err);
  if (ofs) {
    *ofs << "P5\n" << w << " " << h << "\n255\n";
  }
...
std::ofstream *PpmWriter::openGeneric(const char *filename, VideoStitch::ThreadSafeOstream *err) {
